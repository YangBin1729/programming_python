#!python3
# guimaker.py - 一个自动创建窗口中菜单和工具栏的扩展框架
...
class GuiMaker(Frame):
    ...
    def makeMenuBar(self):
        menubar = Frame(self, relief=RAISED, bd=2)      # 基于框架的菜单
        ...
        for (name, key, items) in self.menuBar:
            ...
            self.addMenuItem(pulldown, items)
    ...
    def addMenuItem(self, menu, items):
        for item in items:
            if item == 'separator':
                menu.add_separator({})
            elif type(item) == list:
                ...
                pullover = Menu(menu)
                self.addMenuItem(pullover, item[2])
...
class GuiMakerWindowMenu(GuiMaker):
    def makeMenuBar(self):
        menubar = Menu(self.master)     # 用于顶层窗口的菜单
